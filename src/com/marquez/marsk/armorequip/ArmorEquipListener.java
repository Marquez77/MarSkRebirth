...
public class ArmorEquipListener implements Listener
{
	...
	public final void onInventoryClick(final InventoryClickEvent e) {
		if (e.getClick().equals(ClickType.CREATIVE)) {
			...
			if ((!e.getWhoClicked().getItemOnCursor().equals(null) && e.getSlotType().equals(InventoryType.SlotType.CONTAINER)) || e.getSlotType().equals(InventoryType.SlotType.QUICKBAR)) {
				return;
			}
			...
			if (shift) {
				try {
					...
					if (!(inv.getType().equals(InventoryType.CRAFTING) || inv.getType().equals(InventoryType.PLAYER))) {
						return;
					}
				}
				...
				if (e.getRawSlot() >= e.getInventory().getSize()*9) inv = e.getView().getBottomInventory();
				if (numberkey && (inv.getType().equals(InventoryType.PLAYER) || inv.getType().equals(InventoryType.CRAFTING))) {
					final ItemStack hotbarItem = inv.getItem(e.getHotbarButton());
					if (hotbarItem != null) {
						newArmorType3 = ArmorType.matchType(hotbarItem);
						newArmorPiece = hotbarItem;
					}
					else if (newArmorPiece.getType().equals(Material.AIR)) {
						return;
					}
				}
